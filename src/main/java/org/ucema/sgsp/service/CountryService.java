...
public class CountryService {
	...
	public List<CountryDTO> list() {
		return countryTransformation.transformToApi(countryDAO.findAll());
	}

	@Transactional
	public CountryDTO saveOrUpdate(CountryDTO country) {
		Country response = countryDAO.save(countryTransformation.transformToModel(country));
		country.setId(response.getId());
		return country;
	}	
	...
	public CountryDTO get(Long id){
		...
		return countryTransformation.transformToApi(country);
	}
	...
	public void importGeo(){
		...
		try {
			...
			JSONObject countryStateOptionsProvider = (JSONObject) jsonObject.get("CountryStateOptionsProvider");
			JSONArray optionsArray = (JSONArray) countryStateOptionsProvider.get("options");
			for (Object optionArray : optionsArray) {
				 ...
				 JSONArray admDivisionsArray = (JSONArray) option.get("admDivisions");
				 for (Object admDivisionArray : admDivisionsArray) {
					 JSONObject admDivisionObject = (JSONObject) admDivisionArray;
					 String admDivisionKey = (String) admDivisionObject.get("key");
					 String admDivisionValue = (String) admDivisionObject.get("value");
					 ...
					 JSONArray citiesArray = (JSONArray) admDivisionObject.get("cities");
